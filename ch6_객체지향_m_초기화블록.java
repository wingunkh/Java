/*
	...
	3) 초기화 블록: 초기화 블록 내에서는 메서드 내에서와 같이 조건문, 반복문, 예외처리구문 등을 자유롭게 사용할 수 있으므로,
	초기화 작업이 복잡하여 명시적 초기화만으로는 부족한 경우 사용한다.
	
	[인스턴스 초기화 블록]
	- 용도: 인스턴스변수를 초기화 하는데 사용. (모든 생성자에서 공통으로 수행되어야 하는 코드를 넣는데 사용한다.)
	- 초기화시점: 생성자와 같이 인스턴스를 생성할 때 마다 수행되며, 생성자보다 먼저 수행된다.
	
	[클래스 초기화 블록]
	- 용도: 클래스변수를 초기화 하는데 사용.
	- 초기화시점: 클래스가 메모리에 처음 로딩될 때 한번만 수행된다.
*/
